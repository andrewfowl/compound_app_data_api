...
const COMPOUND_V2_COMPTROLLER_ADDRESS =
  process.env.COMPOUND_V2_COMPTROLLER_ADDRESS ||
  "0x3d9819210A31b4961b30EF54bE2aeD79B9c9Cd3B";
const V2_MARKETS_FILE =
  process.env.COMPOUND_V2_MARKETS_FILE ||
  path.resolve("./data/compound-v2-markets.ethereum.json");
const V3_MARKETS_FILE =
  process.env.COMPOUND_V3_MARKETS_FILE ||
  path.resolve("./data/compound-v3-markets.ethereum.json");
...
function loadUploadedPrices(pathValue: string) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const row = Object.fromEntries(
      header.map((col, idx) => [col, values[idx] ?? ""]),
    );
...
function getUploadedPrice(
  ...
  const byAddress = uploadedPrices.find(
    (row) =>
      row.date === day && row.token_address.toLowerCase() === addr && addr,
  );
  if (byAddress) return byAddress.price_usd;
  const bySymbol = uploadedPrices.find(
    (row) => row.date === day && row.token_symbol.toUpperCase() === sym && sym,
  );
  if (bySymbol) return bySymbol.price_usd;
  return null;
}

async function fetchAlchemyHistoricalPrice(
  tokenAddress: string | null | undefined,
  tokenSymbol: string | null | undefined,
  day: string,
) {
  ...
  try {
    const res = await priceLimit(() =>
      fetch(
        `https://api.g.alchemy.com/prices/v1/${ALCHEMY_PRICES_API_KEY}/tokens/historical`,
        {
          method: "POST",
...
async function resolvePriceFields(
  tokenAddress: string | null | undefined,
  tokenSymbol: string | null | undefined,
  day: string,
) {
  ...
  const alchemy =
    uploaded == null
      ? await fetchAlchemyHistoricalPrice(tokenAddress, tokenSymbol, day)
      : null;
  return {
...
function isRangeLimitError(error: unknown) {
  ...
  return /range limit|10 block range|block range limit exceeded|expanded block range/i.test(
    text,
  );
}

async function fetchLogsRangeAdaptive(
  provider: JsonRpcProvider,
  address: string,
  topics: string[],
  start: number,
  ...
  try {
    ...
    if (isRangeLimitError(error) && start < end) {
      ...
      const left = await fetchLogsRangeAdaptive(
        provider,
        address,
        topics,
        start,
      ...
      const right = await fetchLogsRangeAdaptive(
        provider,
        address,
        topics,
        mid + 1,
...
async function fetchLogsPaged(
  ...
  for (let start = fromBlock; start <= toBlock; start += LOG_PAGE_SIZE) {
    ...
    const logs = await fetchLogsRangeAdaptive(
      provider,
      address,
      topics,
      start,
...
function parseAssetInfo(info: any) {
  return {
    ...
    liquidateCollateralFactor: BigInt(
      info.liquidateCollateralFactor ?? info[5],
    ),
...
async function getExchangeRateStoredCached(
  provider: JsonRpcProvider,
...
async function discoverV2Markets(provider: JsonRpcProvider) {
  const fromFile = loadJsonArray(V2_MARKETS_FILE, false).map((row: any) => ({
    ...
    underlyingAddress: row.underlyingAddress
      ? getAddress(String(row.underlyingAddress))
      : null,
  ...
  for (const cToken of cTokens) {
    ...
    if (!underlyingMaybe) {
      markets.push({
        protocolVersion: "v2",
        marketId: String(cTokenSymbol),
        cTokenAddress,
        cTokenSymbol: String(cTokenSymbol),
        underlyingAddress: null,
        underlyingSymbol: "ETH",
        decimals: 18,
        cTokenDecimals: Number(cTokenDecimals),
      });
      continue;
    }
...
async function inferV2LiquidationUnderlyingFromReceipt(
  ...
  for (const log of receipt.logs) {
    if (log.address.toLowerCase() !== collateralCTokenAddress.toLowerCase())
      continue;
  ...
  if (impliedUnderlyingRaw != null) {
    return {
      ...
      underlyingAmount: round(
        toNumber(impliedUnderlyingRaw, collateralDecimals),
        10,
      ),
    };
  }
...
async function buildV2SnapshotAtBlock(
  provider: JsonRpcProvider,
  address: string,
  blockTag: number,
  markets: any[],
  ...
  const enteredSet = new Set(
    (assetsIn || []).map((x) => String(x).toLowerCase()),
  );
  ...
  for (const market of markets) {
    ...
    const supplyUnderlyingRaw =
      (BigInt(cTokenBalanceRaw as bigint) * BigInt(exchangeRateRaw as bigint)) /
      10n ** 18n;
    const supplyBalance = round(
      toNumber(supplyUnderlyingRaw, market.decimals),
      10,
    );
    const borrowBalance = round(
      toNumber(BigInt(borrowBalanceRaw as bigint), market.decimals),
      10,
    );
    const collateralFactor = Number(
      formatUnits(BigInt((marketTuple as any)?.[1] ?? 0n), 18),
    );
    const enteredAsCollateral = enteredSet.has(
      market.cTokenAddress.toLowerCase(),
    );
    const priceUsd = round(
      formatBigPrice(BigInt(priceRaw as bigint), 36 - market.decimals),
      10,
    );
    ...
    if (supplyBalance !== 0 || borrowBalance !== 0 || enteredAsCollateral) {
      positions.push({
        ...
        marketSymbol: market.cTokenSymbol,
        tokenSymbol: market.underlyingSymbol,
        tokenAddress: market.underlyingAddress,
        enteredAsCollateral,
        collateralFactor: round(collateralFactor, 8),
        supplyBalance,
        supplyUsd,
        borrowBalance,
        borrowUsd,
  ...
  return {
    ...
    positions,
    summary: {
      totalCollateralUsd: round(
        positions.reduce((sum, x) => sum + x.supplyUsd, 0),
        8,
      ),
      totalBorrowUsd: round(
        positions.reduce((sum, x) => sum + x.borrowUsd, 0),
        8,
      ),
      totalBaseSupplyUsd: 0,
      additionalBorrowCapacityUsd: round(
        Number(formatUnits(BigInt(liquidityRaw), 18)),
        8,
      ),
      liquidationBufferUsd: round(
        Number(formatUnits(BigInt(liquidityRaw), 18)),
        8,
      ),
...
async function buildV3SnapshotAtBlock(
  provider: JsonRpcProvider,
  address: string,
  blockTag: number,
  markets: any[],
  ...
  for (const market of markets) {
    ...
    const baseSupplyBalance = round(
      toNumber(BigInt(baseBalanceRaw as bigint), baseDecimals),
      10,
    );
    const borrowBalance = round(
      toNumber(BigInt(borrowBalanceRaw as bigint), baseDecimals),
      10,
    );
    let basePriceUsd = 0;
    if (baseTokenPriceFeed) {
      const basePriceRaw = await safeRpcRead(
        provider,
        market.cometAddress,
        cometIface,
        "getPrice",
      ...
      basePriceUsd = round(
        formatBigPrice(
          BigInt(basePriceRaw as bigint),
          Number(String(priceScale).length - 1),
        ),
        10,
      );
    }
    ...
    for (let i = 0; i < Number(numAssets); i++) {
      const infoRaw = await safeRpcRead(
        provider,
        market.cometAddress,
        cometIface,
        "getAssetInfo",
      ...
      const collateralBalance = round(
        toNumber(BigInt(balanceRaw as bigint), assetDecimals),
        10,
      );
      const priceUsd = round(
        formatBigPrice(
          BigInt(priceRaw as bigint),
          Number(String(priceScale).length - 1),
        ),
        10,
      );
      const collateralUsd = round(collateralBalance * priceUsd, 8);
      const borrowCollateralFactor = round(
        Number(
          formatUnits(
            info.borrowCollateralFactor,
            Number(String(factorScale).length - 1),
          ),
        ),
        8,
      );
      const liquidateCollateralFactor = round(
        Number(
          formatUnits(
    ...
    const additionalBorrowCapacityUsd = round(
      Math.max(0, borrowCapacityUsd - borrowUsd),
      8,
    );
    const liquidationBufferUsd = round(
      Math.max(0, liquidationCapacityUsd - borrowUsd),
      8,
    );
    ...
    if (
      ...
      marketsOut.push({
        ...
        baseTokenAddress: getAddress(String(baseTokenAddress)),
        baseSupplyBalance,
        baseSupplyUsd,
        borrowBalance,
        borrowUsd,
        basePriceUsd,
        additionalBorrowCapacityUsd,
        liquidationBufferUsd,
        isBorrowCollateralized: Boolean(isBorrowCollateralized),
  ...
  return {
    ...
    summary: {
      totalCollateralUsd: round(
        marketsOut
          .flatMap((m) => m.collateralPositions)
          .reduce((sum, x) => sum + x.collateralUsd, 0),
        8,
      ),
      totalBorrowUsd: round(
        marketsOut.reduce((sum, x) => sum + x.borrowUsd, 0),
        8,
      ),
      totalBaseSupplyUsd: round(
        marketsOut.reduce((sum, x) => sum + x.baseSupplyUsd, 0),
        8,
      ),
      additionalBorrowCapacityUsd: round(
        marketsOut.reduce((sum, x) => sum + x.additionalBorrowCapacityUsd, 0),
        8,
      ),
      liquidationBufferUsd: round(
        marketsOut.reduce((sum, x) => sum + x.liquidationBufferUsd, 0),
        8,
      ),
...
function buildUnifiedSummary(v2Snapshot: any, v3Snapshot: any) {
  return {
    totalCollateralUsd: round(
      v2Snapshot.summary.totalCollateralUsd +
        v3Snapshot.summary.totalCollateralUsd,
      8,
    ),
    totalBorrowUsd: round(
      v2Snapshot.summary.totalBorrowUsd + v3Snapshot.summary.totalBorrowUsd,
      8,
    ),
    totalBaseSupplyUsd: round(
      v2Snapshot.summary.totalBaseSupplyUsd +
        v3Snapshot.summary.totalBaseSupplyUsd,
      8,
    ),
    additionalBorrowCapacityUsd: round(
      v2Snapshot.summary.additionalBorrowCapacityUsd +
        v3Snapshot.summary.additionalBorrowCapacityUsd,
      8,
    ),
    liquidationBufferUsd: round(
      v2Snapshot.summary.liquidationBufferUsd +
        v3Snapshot.summary.liquidationBufferUsd,
      8,
    ),
    anyLiquidatable: Boolean(
      v2Snapshot.summary.isLiquidatable || v3Snapshot.summary.anyLiquidatable,
    ),
  };
}
...
async function collectV2NormalizedEvents(
  provider: JsonRpcProvider,
  walletLower: string,
  fromBlock: number,
  toBlock: number,
  ...
  const rawEvents: any[] = [];
  const marketByCToken = new Map(
    markets.map((m) => [m.cTokenAddress.toLowerCase(), m]),
  );
  for (const market of markets) {
    ...
    for (const log of logs) {
      ...
      let matches = false;
      if (parsed.name === "Mint")
        matches = String(parsed.args.minter).toLowerCase() === walletLower;
      if (parsed.name === "Redeem")
        matches = String(parsed.args.redeemer).toLowerCase() === walletLower;
      if (parsed.name === "Borrow")
        matches = String(parsed.args.borrower).toLowerCase() === walletLower;
      if (parsed.name === "RepayBorrow")
        matches = String(parsed.args.borrower).toLowerCase() === walletLower;
      if (parsed.name === "LiquidateBorrow")
        matches = String(parsed.args.borrower).toLowerCase() === walletLower;
      if (!matches) continue;
      ...
      rawEvents.push({
        ...
        day: toDateString(block!.timestamp),
        sourceAction: parsed.name,
        market,
        parsed,
      });
    }
  ...
  rawEvents.sort(
    (a, b) => a.blockNumber - b.blockNumber || a.txHash.localeCompare(b.txHash),
  );
  ...
  for (const e of rawEvents) {
    if (e.sourceAction === "RepayBorrow") {
      const amount = round(
        toNumber(BigInt(e.parsed.args.repayAmount), e.market.decimals),
        10,
      );
  ...
  for (const e of rawEvents) {
    ...
    const price = await resolvePriceFields(
    ...
    if (e.sourceAction === "Mint") {
      const amount = round(
        toNumber(BigInt(e.parsed.args.mintAmount), market.decimals),
        10,
      );
      out.push({
        protocolVersion: "v2",
        ...
        tokenAddress: market.underlyingAddress,
        amount,
        amountUsd: normalizeAmountUsd(amount, price.priceUsd),
    ...
    if (e.sourceAction === "Redeem") {
      const amount = round(
        toNumber(BigInt(e.parsed.args.redeemAmount), market.decimals),
        10,
      );
      out.push({
        protocolVersion: "v2",
    ...
    if (e.sourceAction === "Borrow") {
      const amount = round(
        toNumber(BigInt(e.parsed.args.borrowAmount), market.decimals),
        10,
      );
      out.push({
        protocolVersion: "v2",
    ...
    if (e.sourceAction === "RepayBorrow") {
      const amount = round(
        toNumber(BigInt(e.parsed.args.repayAmount), market.decimals),
        10,
      );
      out.push({
        protocolVersion: "v2",
    ...
    if (e.sourceAction === "LiquidateBorrow") {
      const repayAmount = round(
        toNumber(BigInt(e.parsed.args.repayAmount), market.decimals),
        10,
      );
      if (
        !actualRepays.has(v2RepayKey(e.txHash, market.marketId, repayAmount))
      ) {
        out.push({
          protocolVersion: "v2",
          marketId: market.marketId,
          ...
          positionType: "debt",
          activityType: "repayment",
          sourceAction: "LiquidateBorrow",
          tokenSymbol: market.underlyingSymbol,
          tokenAddress: market.underlyingAddress,
          amount: repayAmount,
          amountUsd: normalizeAmountUsd(repayAmount, price.priceUsd),
          priceUsd: price.priceUsd,
          priceSource: price.priceSource,
          txHash: e.txHash,
          blockNumber: e.blockNumber,
          blockTimestamp: e.blockTimestamp,
          notes: "synthetic debt leg from LiquidateBorrow",
        });
      }
      const collateralCTokenAddress = getAddress(
        String(e.parsed.args.cTokenCollateral),
      );
      const collateralMarket = marketByCToken.get(
        collateralCTokenAddress.toLowerCase(),
      );
      ...
      const fallbackExchangeRate = await getExchangeRateStoredCached(
        provider,
        collateralMarket.cTokenAddress,
        e.blockNumber,
      );
      const inferredCollateral = await inferV2LiquidationUnderlyingFromReceipt(
        provider,
        e.txHash,
        collateralMarket.cTokenAddress,
        liquidator,
      ...
      const collateralPrice = await resolvePriceFields(
        collateralMarket.underlyingAddress,
        collateralMarket.underlyingSymbol,
        e.day,
      );
      out.push({
        protocolVersion: "v2",
        marketId: collateralMarket.marketId,
        marketSymbol: collateralMarket.cTokenSymbol,
        positionType: "collateral",
        activityType: "liquidation",
        sourceAction: "LiquidateBorrow",
        tokenSymbol: collateralMarket.underlyingSymbol,
        tokenAddress: collateralMarket.underlyingAddress,
        amount: inferredCollateral.underlyingAmount,
        amountUsd: normalizeAmountUsd(
          inferredCollateral.underlyingAmount,
          collateralPrice.priceUsd,
        ),
        priceUsd: collateralPrice.priceUsd,
        priceSource: collateralPrice.priceSource,
        txHash: e.txHash,
        blockNumber: e.blockNumber,
        blockTimestamp: e.blockTimestamp,
        notes: `collateral leg valued via ${inferredCollateral.method}`,
      });
    }
  }
  return out;
...
async function collectV3NormalizedEvents(
  provider: JsonRpcProvider,
  walletLower: string,
  fromBlock: number,
  toBlock: number,
  ...
  for (const market of markets) {
    ...
    const baseDecimals = Number(baseDecimalsRaw);
    const assetInfoByAddress = new Map<
      string,
    ...
    for (let i = 0; i < Number(numAssets); i++) {
      ...
      if (!infoRaw) continue;
      const info = parseAssetInfo(infoRaw);
      const symbol = await safeRpcRead(
        provider,
        info.asset,
        erc20Iface,
        "symbol",
      ...
      assetInfoByAddress.set(info.asset.toLowerCase(), {
        address: info.asset,
        symbol: String(symbol),
        decimals: String(info.scale).length - 1,
      });
    }
    ...
    for (const log of logs) {
      let parsed: any;
      try {
        parsed = v3EventIface.parseLog(log);
      } catch {
        continue;
      }
      let include = false;
      if (parsed.name === "Supply")
        include = String(parsed.args.dst).toLowerCase() === walletLower;
      if (parsed.name === "Withdraw")
        include = String(parsed.args.src).toLowerCase() === walletLower;
      if (parsed.name === "SupplyCollateral")
        include = String(parsed.args.dst).toLowerCase() === walletLower;
      if (parsed.name === "WithdrawCollateral")
        include = String(parsed.args.src).toLowerCase() === walletLower;
      if (parsed.name === "AbsorbDebt")
        include = String(parsed.args.borrower).toLowerCase() === walletLower;
      ...
      if (parsed.name === "Supply") {
        const amount = round(
          toNumber(BigInt(parsed.args.amount), baseDecimals),
          10,
        );
        const price = await resolvePriceFields(
          market.baseTokenAddress,
        ...
        out.push({
          ...
          marketSymbol: market.symbol,
          positionType: "base",
          activityType: "base_in",
          sourceAction: "Supply",
          tokenSymbol: market.baseTokenSymbol,
          tokenAddress: market.baseTokenAddress,
          amount,
          amountUsd: normalizeAmountUsd(amount, price.priceUsd),
          priceUsd: price.priceUsd,
          priceSource: price.priceSource,
          ...
          blockNumber: Number(log.blockNumber),
          blockTimestamp: new Date(block!.timestamp * 1000).toISOString(),
          notes: "v3 base_in can mean debt repayment or base supply",
        });
        continue;
      }
      if (parsed.name === "Withdraw") {
        ...
        out.push({
          ...
          marketSymbol: market.symbol,
          positionType: "base",
          activityType: "base_out",
          sourceAction: "Withdraw",
          tokenSymbol: market.baseTokenSymbol,
          tokenAddress: market.baseTokenAddress,
          amount,
          amountUsd: normalizeAmountUsd(amount, price.priceUsd),
          ...
          blockNumber: Number(log.blockNumber),
          blockTimestamp: new Date(block!.timestamp * 1000).toISOString(),
          notes: "v3 base_out can mean borrowing or base withdrawal",
        });
        continue;
      }
      if (parsed.name === "SupplyCollateral") {
        ...
        const amount = round(
          toNumber(BigInt(parsed.args.amount), assetInfo.decimals),
          10,
        );
        const price = await resolvePriceFields(
          assetInfo.address,
      ...
      if (parsed.name === "WithdrawCollateral") {
        ...
        out.push({
          ...
          positionType: "collateral",
          activityType: "redemption",
          sourceAction: "WithdrawCollateral",
          tokenSymbol: assetInfo.symbol,
          tokenAddress: assetInfo.address,
          amount,
          amountUsd: normalizeAmountUsd(amount, price.priceUsd),
      ...
      if (parsed.name === "AbsorbDebt") {
        const amount = round(
          toNumber(BigInt(parsed.args.basePaidOut), baseDecimals),
          10,
        );
        const price = await resolvePriceFields(
          market.baseTokenAddress,
      ...
      if (parsed.name === "AbsorbCollateral") {
        ...
        if (!assetInfo) continue;
        const amount = round(
          toNumber(BigInt(parsed.args.collateralAbsorbed), assetInfo.decimals),
          10,
        );
        const price = await resolvePriceFields(
          assetInfo.address,
...
function summarizeV2Monthly(v2Start: any, v2End: any, normalizedEvents: any[]) {
  ...
  for (const e of normalizedEvents.filter((x) => x.protocolVersion === "v2")) {
    ...
    if (e.positionType === "collateral" && e.activityType === "deposit")
      g.deposits += e.amount;
    if (e.positionType === "collateral" && e.activityType === "redemption")
      g.redemptions += e.amount;
    if (e.positionType === "collateral" && e.activityType === "liquidation")
      g.liquidations += e.amount;
    if (e.positionType === "debt" && e.activityType === "borrowing")
      g.borrowings += e.amount;
    if (e.positionType === "debt" && e.activityType === "repayment")
      g.repayments += e.amount;
  }
  return [...groups.values()].sort(
    (a, b) =>
      a.marketSymbol.localeCompare(b.marketSymbol) ||
...
function summarizeV3Monthly(v3Start: any, v3End: any, normalizedEvents: any[]) {
  ...
  const ensure = (
    ...
    if (!groups.has(key))
      groups.set(key, {
        ...
        beginningBorrow: 0,
        endingBorrow: 0,
        beginningBaseSupply: 0,
        endingBaseSupply: 0,
        collateralDeposits: 0,
        collateralRedemptions: 0,
        collateralLiquidations: 0,
        baseIn: 0,
        baseOut: 0,
        debtLiquidations: 0,
      });
  ...
  for (const market of v3Start.markets) {
    ...
    for (const cp of market.collateralPositions)
      ensure(
        market.marketId,
        market.marketSymbol,
        cp.assetSymbol,
      ).beginningCollateral = cp.collateralBalance;
  }
  for (const market of v3End.markets) {
    ...
    for (const cp of market.collateralPositions)
        ...
        market.marketSymbol,
        cp.assetSymbol,
      ).endingCollateral = cp.collateralBalance;
  }
  for (const e of normalizedEvents.filter((x) => x.protocolVersion === "v3")) {
    const g = ensure(e.marketId, e.marketSymbol, e.tokenSymbol);
    if (e.positionType === "collateral" && e.activityType === "deposit")
      g.collateralDeposits += e.amount;
    if (e.positionType === "collateral" && e.activityType === "redemption")
      g.collateralRedemptions += e.amount;
    if (e.positionType === "collateral" && e.activityType === "liquidation")
      g.collateralLiquidations += e.amount;
    if (e.positionType === "base" && e.activityType === "base_in")
      g.baseIn += e.amount;
    if (e.positionType === "base" && e.activityType === "base_out")
      g.baseOut += e.amount;
    if (e.positionType === "debt" && e.activityType === "liquidation")
      g.debtLiquidations += e.amount;
  }
  return [...groups.values()].sort(
    (a, b) =>
      a.marketSymbol.localeCompare(b.marketSymbol) ||
...
function buildUnifiedEventTotals(events: any[]) {
  return {
    collateralDepositsUsd: round(
      events
        .filter(
          (e) =>
            e.positionType === "collateral" && e.activityType === "deposit",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    collateralRedemptionsUsd: round(
      events
        .filter(
          (e) =>
            e.positionType === "collateral" && e.activityType === "redemption",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    collateralLiquidationsUsd: round(
      events
        .filter(
          (e) =>
            e.positionType === "collateral" && e.activityType === "liquidation",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    debtBorrowingsUsd: round(
      events
        .filter(
          (e) => e.positionType === "debt" && e.activityType === "borrowing",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    debtRepaymentsUsd: round(
      events
        .filter(
          (e) => e.positionType === "debt" && e.activityType === "repayment",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    debtLiquidationsUsd: round(
      events
        .filter(
          (e) => e.positionType === "debt" && e.activityType === "liquidation",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    baseInUsd: round(
      events
        .filter(
          (e) => e.positionType === "base" && e.activityType === "base_in",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
    baseOutUsd: round(
      events
        .filter(
          (e) => e.positionType === "base" && e.activityType === "base_out",
        )
        .reduce((s, x) => s + (x.amountUsd || 0), 0),
      8,
    ),
...
export async function buildCompoundReconciliationReport(params: {
  ...
  await params.onStage(
    "discover_periods",
    `Discovered ${periods.length} monthly periods`,
    periods.length,
    periods.length,
  );
  ...
  for (let i = 0; i < periods.length; i++) {
    ...
    const endBlock = await findBlockByTimestamp(
      provider,
      Math.floor(period.endExclusive.getTime() / 1000) - 1,
    );
    await params.onStage(
      "resolve_blocks",
      `${period.label} blocks ${startBlock}-${endBlock}`,
      i + 1,
      totalPeriods,
    );
    ...
    await params.onStage(
      "snapshots_end",
      `${period.label} end snapshots`,
      i + 1,
      totalPeriods,
    );

    const v2Events = await collectV2NormalizedEvents(
      provider,
      walletLower,
      startBlock,
      endBlock,
    ...
    await params.onStage(
      "fetch_events_v2",
      `${period.label} v2 events`,
      i + 1,
      totalPeriods,
    );

    const v3Events = await collectV3NormalizedEvents(
      provider,
      walletLower,
      startBlock,
      endBlock,
    ...
    await params.onStage(
      "fetch_events_v3",
      `${period.label} v3 events`,
      i + 1,
      totalPeriods,
    );

    const normalizedEvents = [...v2Events, ...v3Events].sort(
      (a, b) =>
        a.blockNumber - b.blockNumber ||
        a.txHash.localeCompare(b.txHash) ||
        a.positionType.localeCompare(b.positionType),
    ...
    await params.onStage(
      "normalize_events",
      `${period.label} normalized ${normalizedEvents.length} events`,
      i + 1,
      totalPeriods,
    );
    ...
    const monthStart = {
      v2: v2MonthStart,
      v3: v3MonthStart,
      unifiedSummary: buildUnifiedSummary(v2MonthStart, v3MonthStart),
    };
    const monthEnd = {
      v2: v2MonthEnd,
      v3: v3MonthEnd,
      unifiedSummary: buildUnifiedSummary(v2MonthEnd, v3MonthEnd),
    };
    ...
    results.push({
      ...
      reconciliationSummary: [
        ...monthlyRollforward.v2,
        ...monthlyRollforward.v3,
        {
          protocolVersion: "unified",
          marketId: null,
          marketSymbol: "all",
          tokenSymbol: "USD",
          ...monthlyRollforward.unifiedEventTotalsUsd,
        },
      ],
    });
    await params.onStage(
      "reconcile_periods",
      `${period.label} summarized`,
      i + 1,
      totalPeriods,
    );
  }
